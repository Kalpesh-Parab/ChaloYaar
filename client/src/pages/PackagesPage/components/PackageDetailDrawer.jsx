...
import { X, Check, Clock, Calendar, MapPin, MessageCircle, ArrowRight } from 'lucide-react';

export default function PackageDetailDrawer({ pkg, onClose, whatsappNumber = "8446257619" }) {
  if (!pkg) return null;
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-coal-950/60 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-xl bg-coal-950 text-white h-full overflow-y-auto p-6 sm:p-8 flex flex-col justify-between shadow-2xl border-l border-coal-800">
        ...
        <div>
          ...
          <div className="mt-6 space-y-3">
            ...
            {pkg.itinerary?.map((day, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-coal-900 border border-coal-800 flex gap-3">
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {pkg.inclusions.map((inc, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-sand-100">
        ...
        <div className="pt-6 mt-8 border-t border-coal-800 flex items-center justify-between gap-4">
          <div>
            <span className="text-[11px] text-sand-400 block font-sans">Total Investment</span>
            <span className="font-sans font-extrabold text-2xl text-brand-yellow">{pkg.price}</span>
